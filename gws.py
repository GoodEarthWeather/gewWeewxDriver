...
def getSimplePacket(pkt, data):
    ...
    h = abs(pkt[3]) * 125.0 / 65535.0 - 6.0
    if h > 100.0:
        h = 100.0
    if h < 0.0:
        h = 0.0
    data['outHumidity'] = h
    data['outTemp'] = (pkt[4] * 175.0 / 65535.0 - 45.0) * 1.8 + 32.0
    ...
    # need to get etime
    #
    battery = (pkt[12] / 4096.0) * 1.2 * (4.6)
    solarV = (pkt[13] / 4096.0) * 1.2 * (4.6)
    data['supplyVoltage'] = solarV
...
def getHT(pkt, command, data):
    h = pkt[3] * 125.0 / 65535.0 - 6.0
    t = (pkt[4] * 175.0 / 65535.0 - 45.0) * 1.8 + 32.0
    if h > 100.0:
        h = 100.0
    if h < 0.0:
        h = 0.0
...
def getPower(pkt, data):
    battery = (pkt[3] / 4096.0) * 1.2 * (4.6)
    solarV = (pkt[4] / 4096.0) * 1.2 * (4.6)
    data['supplyVoltage'] = solarV
    data['referenceVoltage'] = battery
...
class GWSDriver(weewx.drivers.AbstractDevice):
    ...
    cmdFormat[17] = '=3BLL'  # 'CMD_RTN_PRESSURE'
